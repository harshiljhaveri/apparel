...
print(modellib.__file__ ,utils.__file__ ,dir(modellib) )
...
class BalloonDataset(utils.Dataset):
    ...
    def load_balloon(self, dataset_dir, subset):
        ...
        self.add_class("balloon", 1, "short sleeve top")
        self.add_class("balloon", 2, "long sleeve top")
        self.add_class("balloon", 3, "short sleeve outwear")
        self.add_class("balloon", 4, "long sleeve outwear")
        self.add_class("balloon", 5, "vest")
        self.add_class("balloon", 6, "sling")
        self.add_class("balloon", 7, "shorts")
        self.add_class("balloon", 8, "trousers")
        self.add_class("balloon", 9, "skirt")
        self.add_class("balloon", 10, "short sleeve dress")
        self.add_class("balloon", 11, "long sleeve dress")
        self.add_class("balloon", 12, "vest dress")
        self.add_class("balloon", 13, "sling dress")
        # Train or validation dataset?
        assert subset in ["train", "val"]
        dataset_dir = os.path.join(dataset_dir, subset)
        dataset_img = os.path.join(dataset_dir,'image')
        dataset_annos = os.path.join(dataset_dir,'data.json')
        # Load annotations
...
#         annotations = []
#         for j in glob.glob("/home/ai/Desktop/fashion/fashion/train/annos/" + '*.json'):
#             with open(j) as f:
#               data = json.load(f)
#             for i in data.keys():
#                 if i != 'source' and i != 'pair_id':# don't need the dict keys
#                         annotations.append(data.get(i).)
...
#                 polygons = [r['shape_attributes'] for r in a['regions']] 
        ...
        with open(dataset_annos) as f:
            ...
            for filename, data in all_data.items():
                ...
                for i in data.keys():
                    if i != 'source' and i != 'pair_id' and i != 'height' and i != 'width':
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        x = polygon.get('x')
        y = polygon.get('y')
        mask = np.zeros([info["height"], info["width"], len(x)],
                        dtype=np.uint8)

        for i, p in enumerate(x):
            print(i,p)
            # Get indexes of pixels inside the polygon and set them to 1
            rr, cc = skimage.draw.polygon(y[i][:], p[:])
            mask[rr, cc, i] = 1
